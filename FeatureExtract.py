...
'''
visitorFeatures()-- features:
-most common action
-time elapsed per visit
-number of actions conducted per visitor
...
def visitorFeatures(api_data):
    ...
    for value in visitors_data:
        ...
        for user_action in value['events']['list']:
            actions[0][user_action[2]] += 1 #if the action is present, add at index
    ...
    key_features = {'most freq' : 0 , 'normalized time' : 1, 'actions' : 2}

    return x, key_features

'''
nextActionFeaturesWithTime(api_data, act_clusters)
...
'''
nextActionFeatures(api_data, act_clusters)
determines type of visit by finding
-current action
-immediately proceeding action
...
def nextActionFeatures(api_data, act_clusters):
    #append timestamp(ms), elapsed_time, act#, action, n_axt#, next_action
    action_dict = api_data.actions
    visitors_data = api_data.visitors
    ...
    x = np.hstack((x, predictions[:,None]))
    np.savetxt("action_clusters.txt",x, fmt='%i')
    ...
    for value in visitors_data:
        ...
        for user_action in value['events']['list']:
            index = np.array([user_action[2], user_action[4]]).reshape(1,-1)
            label = action_model.predict(index)
            actions.append(label)
            total_time += (user_action[0]- base_time)

        features[0][0] = mode(actions) #most frequent action
        features[0][1] = total_time / value['events']['count'] #average time between events
        features[0][2] = value['events']['count'] #number of events
        x.append(features)
    ...
    key_features = {'visit label' : 0 , 'normalized_time':1, 'actions' : 2}
